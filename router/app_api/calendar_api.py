...
@router.get('', tags=['calendar'], summary='달력 한달치 수업 목록', dependencies=[Depends(get_current_user)])
def get_calendar_lesson(session: Session = Depends(db.session),
                        month: Optional[int] = None):
    result_msg = '달력 한달치 수업 목록'
    try:
        response = calendar_controller.get_calendar_lesson(session=session,
                                                           month=month)
