...
class PostChatRoomExistsModel(BaseModel):
    user_id: int
...
@router.post('/{chat_room_id}/notice', tags=['chat_room'], summary='채팅방 알림 on/off', dependencies=[Depends(get_current_user)])
def post_chat_room_detail_notice(chat_room_id: int,
                                 session: Session = Depends(db.session),
                                 g: User = Depends(get_current_user)):
    result_msg = '채팅방 알림 on/off'
    try:
        response = chat_room_controller.post_chat_room_detail_notice(chat_room_id=chat_room_id,
                                                                     session=session,
                                                                     g=g)
    except HTTPException as e:
        print(f'error: {e.detail}')
...
@router.post('/exists', tags=['chat_room'], summary='채팅방 확인', dependencies=[Depends(get_current_user)])
def post_chat_room_exists(request: PostChatRoomExistsModel,
                          session: Session = Depends(db.session),
                          g: User = Depends(get_current_user)):
    result_msg = '채팅방 확인'
    try:
        response = chat_room_controller.post_chat_room_exists(request=request,
                                                              session=session,
                                                              g=g)
    except HTTPException as e:
        print(f'error: {e.detail}')
