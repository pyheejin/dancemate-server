...
class PostCourseDetailReviewModel(BaseModel):
    rate: int
    description: str
...
@router.post('/{course_id}/review', tags=['course'], summary='리뷰 작성', dependencies=[Depends(get_current_user)])
def post_course_detail_review(course_id: int,
                              request: PostCourseDetailReviewModel,
                              session: Session = Depends(db.session),
                              g: User = Depends(get_current_user)):
    result_msg = '리뷰 작성'
    try:
        response = course_controller.post_course_detail_review(course_id=course_id,
                                                               request=request,
                                                               session=session,
                                                               g=g)
    except HTTPException as e:
