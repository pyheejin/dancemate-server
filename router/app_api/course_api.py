...
@router.put('/{course_id}', tags=['course'], summary='수업 수정', dependencies=[Depends(get_current_user)])
...
@router.post('/{course_detail_id}/cancel', tags=['course'], summary='수업 예약 취소', dependencies=[Depends(get_current_user)])
def post_course_detail_cancel(course_detail_id: int,
                              session: Session = Depends(db.session),
                              g: User = Depends(get_current_user)):
    result_msg = '수업 예약 취소'
    try:
        response = course_controller.post_course_detail_cancel(course_detail_id=course_detail_id,
                                                               session=session,
                                                               g=g)
    except HTTPException as e:
        print(f'error: {e.detail}')
...
@router.post('/{course_detail_id}/exists', tags=['course'], summary='예약 가능 여부 확인', dependencies=[Depends(get_current_user)])
def post_course_detail_exists(course_detail_id: int,
                              session: Session = Depends(db.session),
                              g: User = Depends(get_current_user)):
    result_msg = '예약 가능 여부 확인'
    try:
        response = course_controller.post_course_detail_exists(course_detail_id=course_detail_id,
                                                               session=session,
                                                               g=g)
    except HTTPException as e:
        print(f'error: {e.detail}')
