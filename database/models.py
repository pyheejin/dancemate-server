...
class User(Base):
    ...
    method = Column(Integer, default=constant.USER_METHOD_DEFAULT, comment='1:일반, 2:구글, 3:카카오, 4:애플')
    email = Column(String(255), comment='이메일')
    password = Column(Text, comment='비밀번호')
    ...
    access_token = Column(Text, comment='')
    refresh_token = Column(Text, comment='')
    ...
    chat_room = relationship('ChatRoom', back_populates='user', foreign_keys='[ChatRoom.user_id]')
    chat_room_friend = relationship('ChatRoom', back_populates='friend', foreign_keys='[ChatRoom.friend_id]')
...
class ChatRoom(Base):
    ...
    user_id = Column(Integer, ForeignKey('user.id'), comment='')
    friend_id = Column(Integer, ForeignKey('user.id'), comment='DM일 때 상대방 id')
    lesson_id = Column(Integer, ForeignKey('lesson.id'), comment='')
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)

    user = relationship('User', back_populates='chat_room', foreign_keys=[user_id])
    friend = relationship('User', back_populates='chat_room_friend', foreign_keys=[friend_id])
